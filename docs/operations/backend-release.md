...
`20260810001100_class_scoped_join_preparation.sql`. The final readiness
migration verifies the complete chain, the one-round-trip class-scoped join
preparation RPC, class-access column and unique index, private credential/attempt
table ownership, RLS and browser ACLs, plus the exact owner-only private-table
and owner-plus-service join/login RPC ACL allowlists, including grant-option
state. Configure the encrypted student-login signer only
